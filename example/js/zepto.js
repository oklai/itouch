...
;(function($){
  ...
  ;$.fn["swipeListener"]=function(events,callback){
  	var that=this;
  	events.split(/\s/).forEach(function(m){
  		swipeListener(m);
  	});
  	function swipeListener(direction){
  		var direction=direction.toLowerCase();
	  	if(direction==='swipe') return that.swipe(callback);
	  	if(direction==='swipeleft') return that.swipeLeft(callback);
	  	if(direction==='swiperight') return that.swipeRight(callback);
	  	if(direction==='swipeup') return that.swipeUp(callback);
	  	if(direction==='swipedown') return that.swipeDown(callback);
  	}
  }
})(Zepto)
